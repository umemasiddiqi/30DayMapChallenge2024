...
# 1. Load the data -------------------------------------------------------------
streets <- getbb('Auckland New Zealand') %>%
  opq() %>%
...
small_streets <- getbb('Auckland New Zealand') %>%
  opq() %>%
  add_osm_feature(key='highway',
...
rivers <- getbb('Auckland New Zealand') %>%
  opq() %>%
...
ggplot() +
  ...
  coord_sf(ylim=c(-36.80, -36.87),
           xlim=c(174.73, 174.80),
           expand=FALSE)+
  
  #2b. To add the border and title etc.
  labs(title = "Auckland", caption = "#30DayMapChallenge2024 | Day 2: Lines | Designer: U. Siddiqi") + 
  theme(
...
ggsave(paste0("Auck_Lines_", format(Sys.time(), "%d%m%Y"), ".jpg"), dpi = 320, width = 6, height = 6)
